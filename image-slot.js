...
(() => {
  ...
  function load() {
    ...
    const stateUrl = (typeof window !== 'undefined' && window.__resources && window.__resources.imageSlots) || STATE_FILE;
  ...
  async function toDataUrl(file, targetW) {
    ...
    try {
      const cap = Math.min(MAX_DIM, Math.max(1, Math.round(targetW * 3)) || MAX_DIM);
